...
double vxdot(double x,double y,double vy,double xsat, double xmimas, double omega);
double vydot(double x,double y,double vx,double xsat, double xmimas, double omega);
void initcond();
...
double a = 189176000; //separation of mimas + saturn in meters
...
int main(){
  ...
  for(i=0;i<10;i++){
	...
	while(t1<tf){
	  ...
	  k1 = vxdot(x1,yy1,vy1,xsat, xmimas, omega)*dt;
	  l1 = vydot(x1,yy1,vx1,xsat, xmimas, omega)*dt;
	  m1 = vx1*dt;
	  n1 = vy1*dt;
	  
	  k2 = vxdot((x1+.5*m1),(yy1+.5*n1),(vy1 + .5*l1),xsat, xmimas, omega)*dt;
	  l2 = vydot((x1+.5*m1),(yy1+.5*n1),(vx1 + .5*k1),xsat, xmimas, omega)*dt;
	  m2 = (vx1 + .5*k1)*dt;
	  n2 = (vy1 + .5*l1)*dt;
	  
	  k3 = vxdot((x1+.5*m2),(yy1+.5*n2),(vy1 + .5*l2),xsat, xmimas, omega)*dt;
	  l3 = vydot((x1+.5*m2),(yy1+.5*n2),(vx1 + .5*k2),xsat, xmimas, omega)*dt;
	  m3 = (vx1 + .5*k2)*dt;
	  n3 = (vy1 + .5*l2)*dt;
	  
	  k4 = vxdot((x1+m3),(yy1+n3),(vy1+l3),xsat, xmimas, omega)*dt;
	  l4 = vydot((x1+m3),(yy1+n3),(vx1+k3),xsat, xmimas, omega)*dt;
	  m4 = (vx1 + k3)*dt;
	  n4 = (vy1 + l3)*dt;
...
void initcond(){
  ...
  x1 = 0.0;
  yy1 = 65232000.0+ rn*73000000.0;   //random numbers in range of saturn's rings.  in KILOMETERS
  vy1 = 0.0;    //starts at rest in y direction so it is moving tangential to saturn;
  rn = ran1(&idum);
  vx1 = (29.4/pow(yy1,.5))*1000.0;
